...
# 引入我们刚才写的业务路由
from app.api.routers import record,user,audio
from app.database import create_db_and_tables, seed_test_data

# 定义 FastAPI 启动和关闭时的动作
@asynccontextmanager
async def lifespan(app: FastAPI):
    print("⏳ 正在初始化数据库...")
    create_db_and_tables() # 自动建表
    seed_test_data()       # 自动注入测试账号
    print("✅ 数据库准备就绪！")
    yield
    print("🛑 服务器正在关闭...")

# 实例化 FastAPI，这里的 title 和 description 会直接展示在接口文档的网页头上！
app = FastAPI(
    title="AI 警讯笔录系统 API",
